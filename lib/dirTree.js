 ...
 * 功能：序列化辅助函数
 * 作者：guzheyuan
 * 输入：obj [obj]  --树形json对象的节点，目录／文件
 *      depth [int]  --遍历的深度
 *      isEnd [arr]  --bool数组，表示各级是否是文件夹中最后一个
 * 输出：str [string]  --返回序列化obj后的字符串 
 */
function stringifyHelper(obj, depth, isEnd) {
    var tmpStr = "";
    ...
    if(obj["type"] == "dir") {
        ...
        for(let i = 0; i < obj["childD"].length; i++) {
            ...
            tmpStr += stringifyHelper(obj["childD"][i], depth+1, isEnd);
        }
        for(let i = 0; i < obj["childF"].length; i++) {
            isEnd[depth+1] = (--arrLength ? false : true);
            tmpStr += stringifyHelper(obj["childF"][i], depth+1, isEnd);
        }
 ...
 */
function stringifyDirTree(dirTree) {
    var str = "";
    var depth = 0;
    ...
    for(let i = 0; i < dirTree["childD"].length; i++) {
        isEnd[depth+1] = --arrLength ? false : true;
        str += stringifyHelper(dirTree["childD"][i], depth+1, isEnd);
    }
    for(let i = 0; i < dirTree["childF"].length; i++) {
        isEnd[depth+1] = --arrLength ? false : true;
        str += stringifyHelper(dirTree["childF"][i], depth+1, isEnd);
    }
...
// test
var tree = dirDFS("../../TapAdServer", "TapAdServer", [/^\./, /node_modules/]);
var str = stringifyDirTree(tree);
console.log(str);
